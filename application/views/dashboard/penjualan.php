        ...
        ***********************************-->
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <button type="button" class="btn btn-rounded btn-info" id="generateReport">
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-12">
                                        <button id="exportButton" class="btn btn-success btn-block">Ekspor ke
                                            Excel</button>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <!-- FILTER TANGGAL ADA 2  -->
                                <div class="row">
                                    <div class="col-md-4">
                                        <label for="tanggal">Filter Per Hari</label>
                                ...
                                <div class="table-responsive">
                                                    ...
                                                    <td>
                                                        <!-- <div class="d-flex">
                                                            <a href="" data-bs-toggle="modal"
                                                                data-bs-target="#detailProduk<?= $data['id_transaksi'] ?>"
                                    ...
                                    <!-- <div class="modal fade" id="detailProduk<?= $data['id_transaksi'] ?>" tabindex="-1"
                                                            ...
                                                            <table class="table">
                                                                ...
                                                                <tbody id="detailBody">
                                                                    <tr>
                                                                        ...
                                                                        <td><?= number_format($item->potongan, 2); ?>
                                                                        </td>
        ...
        <script>
            document.getElementById('generateReport').addEventListener('click', function () {
                // Ambil konten tabel dari elemen dengan ID 'example'
                ...
                win.document.write('table { width: 100%; border-collapse: collapse; margin: 20px 0; }');
                ...
                win.document.write('tr:hover { background-color: #f1f1f1; }');
                win.document.write('</style>');
                win.document.write('</head><body>');
                win.document.write('<h1>Laporan Penjualan</h1>');

                // Ambil semua baris dari tabel
                var rows = document.querySelectorAll('#example tbody tr');
                var numberedRows = '';

                // Loop untuk menambahkan nomor urut
                rows.forEach(function (row, index) {
                    // Buat salinan baris dan tambahkan nomor urut
                    var newRow = row.cloneNode(true);
                    newRow.cells[0].innerText = index + 1; // Set nomor urut mulai dari 1
                    numberedRows += newRow.outerHTML; // Tambahkan baris yang sudah dinomori
                });

                // Tulis baris yang sudah dinomori ke jendela cetak
                win.document.write('<table class="table table-striped table-responsive-sm">');
                win.document.write('<thead class="thead-primary">' + document.querySelector('#example thead').outerHTML + '</thead>');
                win.document.write('<tbody>' + numberedRows + '</tbody>');
                win.document.write('</table>');

                win.document.write('</body></html>');

                // Selesai menulis, cetak jendela
                win.document.close(); // Penting untuk menutup dokumen
                win.print();
            });
        </script>
        ...
        <script>
            document.getElementById('filterButton').addEventListener('click', function () {
                ...
                rows.forEach(function (row) {
                    var rowDate = row.cells[1].innerText; // Ambil tanggal dari kolom kedua
                    var rowDateOnly = rowDate.split(' ')[0]; // Ambil hanya bagian tanggal (YYYY-MM-DD)

                    // Ambil tahun-bulan dari tanggal
                    var rowMonth = rowDateOnly.substring(0, 7); // Ambil tahun-bulan dari tanggal

                    // Cek apakah baris sesuai dengan filter
                    var dateMatch = selectedDate ? rowDateOnly === selectedDate : true;
                    var monthMatch = selectedMonth ? rowMonth === selectedMonth : true;
        ...
        <script>
            document.getElementById('exportButton').addEventListener('click', function () {
                // Ambil nilai filter tanggal dan bulan
                const tanggal = document.getElementById('tanggal').value;
                const bulan = document.getElementById('bulan').value;

                // Buat URL untuk mengakses fungsi ekspor di controller
                let url = '<?= base_url('Extrak/') ?>';
                ...
                if (bulan) {
                    url += (tanggal ? '&' : '?') + 'bulan=' + bulan;
                }

                // Redirect ke URL untuk mengunduh file Excel
    ...
    ***********************************-->
        ...
        function deleteConfirm(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "<?= base_url('Delete_kategori/') ?>" + id;
                }
            });
        }
    </script>
    ...
    <script>
        $(document).ready(function () {
            $("input[name='tanggal'], input[name='bulan']").on('change', function () {
                let selectedDate = $("input[name='tanggal']").val();
                let selectedMonth = $("input[name='bulan']").val();

                $("#example tbody tr").each(function () {
                    let rowDate = $(this).find("td:nth-child(2)").text().trim(); // Ambil tanggal dari kolom kedua
                    ...
                    if (selectedDate) {
                        ...
                        let rowMonth = rowDate.substring(0, 7); // Ambil format YYYY-MM dari tanggal
                        $(this).toggle(rowMonth === selectedMonth);
                    } else {
                        $(this).show(); // Tampilkan semua jika tidak ada filter
                    }
                });
            });
        });
